...
public class MasterPromotePermissions implements Listener {
	...
	private Plugin lp = Bukkit.getPluginManager().getPlugin("LuckPems");
	...
	public void loadPermission() {
		...
		} else if (Bukkit.getPluginManager().isPluginEnabled(lp)) {
			this.activePermissions = "LuckPerms";
		} else {
	...
	public void promote(Player player, String group, PROMOTIONTYPE type) {
		...
		} else if (activePermissions.equalsIgnoreCase("LuckPerms")) {
			Bukkit.dispatchCommand(console, "lp user " + player.getName() + " parent set " + group
					+ " " + plugin.config.getString("LuckPermsContext", ""));
		}
	}
